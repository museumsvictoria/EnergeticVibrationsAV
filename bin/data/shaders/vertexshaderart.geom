...
void main() {
    ...
    for (int i = 0; i < gl_in.length(); i++)
    {
        ...
        vec4 tri_height = mix(vertex[i].position, vec4(vMiddle,1.0) + vertex[i].position + vec4(normal.xyz,1.0) *15.0, lfo_scale);
//        vec4 tri_height = mix(vertex[i].position, (vertex[i].position-(vec4(vMiddle,1.0))*.01) + vec4(normal.xyz,1.0) *5.0, lfo_scale);
